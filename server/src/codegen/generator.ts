...
export async function generate(options: CodegenOptions) {
  ...
  Object.entries(options.openAPI.paths).map(([path, value]: [string, PathItemObject]) => {
...
function handler(options: CodegenOptions, controllers: ByController, item: Item) {
  ...
  let targetDir = path.resolve(options.basePath, relativePath);

  fs.mkdirpSync(targetDir);

  if (fileName.includes('#controller')) {
    Object.entries(controllers).map(([key, value]: [string, ControllerInfo]) => {
      let targetName = fileName.replace(/#controller/, value.fileName);
      handleFile(
        item.path,
